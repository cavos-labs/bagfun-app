...
export default function CreateTokenModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (hasDirectWallet) {
        ...
        const txResult = await cavosAuth.executeCalls(
          user.wallet_address,
          calls,
          user.access_token
        );
        ...
        const ownershipTransferredEvent = txDetails.data.receipt.events.find(
          (event: any) => event.name === "OwnershipTransferred"
        );

        if (!ownershipTransferredEvent) {
          throw new Error(
            "OwnershipTransferred event not found in transaction"
          );
        }
        ...
        await cavosAuth.executeCalls(
          user.wallet_address,
          launchCalls,
          txResult.accessToken
        );
      ...
      result = await TokenService.createToken(tokenData);
  ...
  useEffect(() => {
    if (
      isOpen &&
      (user?.wallet_address || (isWalletConnected && walletAddress))
    ) {
      fetchStarkBalance();
    }
